...
async function checkDataIntegrity() {
  try {
    ...
    // Check bookings table integrity
    const bookingRepository = dataSource.getRepository(Booking);
    const bookings = await bookingRepository.find();
    console.log(`\n📊 Checking ${bookings.length} bookings for data integrity:`);
    let bookingIssues = 0;
    for (const booking of bookings) {
      const user = await dataSource.getRepository(User).findOne({ where: { id: booking.userId } });
      if (!user) {
        console.log(`❌ Booking ID ${booking.id} references non-existent user ID ${booking.userId}`);
        bookingIssues++;
      }
    }
    if (bookingIssues === 0) {
      console.log('✅ All bookings reference valid users');
    }
    ...
    // Summary
    const totalIssues = workerIssues + bookingIssues + serviceRequestIssues + waitlistIssues + assignmentMetricsIssues + userBehaviorMetricsIssues;
    if (totalIssues === 0) {
