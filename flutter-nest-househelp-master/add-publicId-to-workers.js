...
async function addPublicIdColumn() {
  try {
    console.log('Adding publicId column to workers table...');
    ...
    const result = await pool.query('SELECT id FROM "worker"');
    for (const row of result.rows) {
      const uuid = require('crypto').randomUUID();
      await pool.query(
        'UPDATE "worker" SET "publicId" = $1 WHERE id = $2',
        [uuid, row.id]
      );
    }
    
    console.log('Existing workers updated with UUIDs');
    ...
    await pool.query(`
      ALTER TABLE "worker" 
      ADD CONSTRAINT "UQ_worker_publicId" UNIQUE ("publicId")
    `);
