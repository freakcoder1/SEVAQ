...
async function addPublicIdColumn() {
  try {
    console.log('Adding publicId column to service_requests table...');
    ...
    const result = await pool.query('SELECT id FROM "service_requests"');
    for (const row of result.rows) {
      const uuid = require('crypto').randomUUID();
      await pool.query(
        'UPDATE "service_requests" SET "publicId" = $1 WHERE id = $2',
        [uuid, row.id]
      );
    }
    
    console.log('Existing service requests updated with UUIDs');
    ...
    await pool.query(`
      ALTER TABLE "service_requests" 
      ADD CONSTRAINT "UQ_service_requests_publicId" UNIQUE ("publicId")
    `);
