...
const API_BASE = 'http://0.0.0.0:45357';

async function testAssignmentWithSpecificSlot() {
    console.log('=== TESTING ASSIGNMENT WITH SPECIFIC AVAILABLE SLOT ===\n');

    try {
        // 1. Create a test booking with the specific available slot
        console.log('1. Creating a test booking with specific available slot...');
        const bookingData = {
            ...
            amount: 500,
            type: 'on_demand',
            notes: 'Test booking for assignment with specific slot'
        };
        ...
        // 5. Check if worker was assigned
        if (assignmentResponse.data.assignedWorkerId) {
            console.log('\n✅ SUCCESS: Worker was assigned!');
            console.log(`Worker ID: ${assignmentResponse.data.assignedWorkerId}`);
        } else {
            console.log('\n❌ FAILED: No worker was assigned');
        }
...
testAssignmentWithSpecificSlot();