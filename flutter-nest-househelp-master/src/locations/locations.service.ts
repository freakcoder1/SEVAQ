...
export class LocationService {
  ...
  async findNearbyZones(lat: number, lng: number, maxRadiusKm: number = 2): Promise<MicroZone[]> {
    this.logger.debug(`Finding nearby zones for coordinates: lat=${lat}, lng=${lng}, maxRadius=${maxRadiusKm}km`);
    ...
    try {
      ...
      const nearbyZones = zones.filter(zone => {
        ...
        this.logger.debug(`Zone ${zone.name}: distance=${distance.toFixed(2)}km, isNearby=${isNearby}`);
        return isNearby;
      });

      this.logger.log(`Returning ${nearbyZones.length} nearby zones for lat=${lat}, lng=${lng}`);
      return nearbyZones;
    } catch (error) {
      this.logger.error(`Error finding nearby zones: ${error.message}`, error.stack);
      throw error;
    }
  }
  ...
  async checkServiceAvailability(lat: number, lng: number, radiusKm: number = 5): Promise<{
    ...
    this.logger.log(`Checking service availability for lat=${lat}, lng=${lng}, radius=${radiusKm}km`);

    try {
      const nearbyWorkers = await this.findAvailableWorkers(lat, lng, radiusKm);
      const nearbyZones = await this.findNearbyZones(lat, lng);

      this.logger.debug(`Found ${nearbyWorkers.length} workers and ${nearbyZones.length} zones`);
      ...
      this.logger.log(`Service availability result: isAvailable=${result.isAvailable}, workerCount=${workerCount}, estimatedWaitTime=${estimatedWaitTime}min`);
      return result;
    } catch (error) {
      this.logger.error(`Error checking service availability: ${error.message}`, error.stack);
      throw error;
    }
  }
  ...
  async findAvailableWorkers(lat: number, lng: number, radiusKm: number): Promise<Worker[]> {
    this.logger.debug(`Finding available workers for lat=${lat}, lng=${lng}, radius=${radiusKm}km`);
    ...
    try {
      ...
      this.logger.error(`Error finding available workers: ${error.message}`, error.stack);
      throw error;
    }
  }
  ...
  async getAvailableServices(lat: number, lng: number, radiusKm: number = 5): Promise<any[]> {
    this.logger.log(`Getting available services for lat=${lat}, lng=${lng}, radius=${radiusKm}km`);
    ...
    try {
      ...
      this.logger.error(`Error getting available services: ${error.message}`, error.stack);
      throw error;
    }
  }
  ...
  async getWaitlistStatus(userId: string): Promise<Waitlist[]> {
    ...
    try {
      ...
      this.logger.debug(`Found ${entries.length} pending waitlist entries for user ${userId}`);
      return entries;
    } catch (error) {
      this.logger.error(`Error getting waitlist status: ${error.message}`, error.stack);
      throw error;
    }
  }
  ...
  async cancelWaitlist(waitlistId: string): Promise<void> {
    ...
    try {
      ...
      this.logger.error(`Error cancelling waitlist entry: ${error.message}`, error.stack);
      throw error;
    }
  }
  ...
  async removeFromWaitlist(userId: string): Promise<void> {
    ...
    try {
      ...
      this.logger.log(`Removed ${result.affected || 0} waitlist entries for user ${userId}`);
    } catch (error) {
  ...
  async updatePreferredLocation(userId: string, lat: number, lng: number): Promise<User> {
    ...
    try {
      ...
      this.logger.log(`Successfully updated preferred location for user ${userId}`);
      return saved;
    } catch (error) {
      if (error instanceof NotFoundException) throw error;
      this.logger.error(`Error updating preferred location: ${error.message}`, error.stack);
      throw error;
    }
  }
  ...
  async updateWorkerLocation(workerId: string, lat: number, lng: number): Promise<Worker> {
    ...
    try {
      ...
      if (lat < -90 || lat > 90 || lng < -180 || lng > 180) {
        this.logger.warn(`Invalid coordinates received: lat=${lat}, lng=${lng}`);
      ...
      this.logger.log(`Successfully updated current location for worker ${workerId}`);
      return saved;
    } catch (error) {
      if (error instanceof NotFoundException || error instanceof BadRequestException) throw error;
      this.logger.error(`Error updating worker location: ${error.message}`, error.stack);
      throw error;
    }
  }
  ...
  async getServiceAreasForLocation(lat: number, lng: number): Promise<ServiceArea[]> {
    this.logger.debug(`Getting service areas for location: lat=${lat}, lng=${lng}`);
  ...
  async createMicroZone(data: {
    ...
    try {
      ...
      if (data.radiusKm <= 0 || data.radiusKm > 100) {
        throw new BadRequestException('Invalid radius: must be between 0 and 100 km');
      }

      const zone = this.microZoneRepository.create(data);
      const saved = await this.microZoneRepository.save(zone);
      this.logger.log(`Successfully created micro-zone: ${saved.id} - ${data.name}`);
      return saved;
    } catch (error) {
      if (error instanceof BadRequestException) throw error;
      this.logger.error(`Error creating micro-zone: ${error.message}`, error.stack);
      throw error;
    }
  }
  ...
  async createServiceArea(data: {
    ...
    try {
      ...
      if (data.minLat > data.maxLat || data.minLng > data.maxLng) {
        throw new BadRequestException('Invalid bounds: min values must be less than max values');
      }

      const area = this.serviceAreaRepository.create(data);
      const saved = await this.serviceAreaRepository.save(area);
      this.logger.log(`Successfully created service area: ${saved.id} - ${data.name}`);
      return saved;
    } catch (error) {
      if (error instanceof BadRequestException) throw error;
  ...
  async updateAllWorkersToLocation(lat: number, lng: number): Promise<{ updated: number; errors: number }> {
    this.logger.log(`Setting default location (lat=${lat}, lng=${lng}) for all workers without location`);
      ...
      for (const worker of workers) {
        if (!worker.currentLat || !worker.currentLng) {
          try {
            ...
            updated++;
            this.logger.debug(`Updated worker ${worker.id} to default location`);
          } catch (error) {
            errors++;
            this.logger.error(`Failed to update worker ${worker.id}: ${error.message}`);
          }
        }
      }

      this.logger.log(`Worker location update complete: ${updated} updated, ${errors} errors`);
      return { updated, errors };
    } catch (error) {
      this.logger.error(`Error updating all workers: ${error.message}`, error.stack);
      throw error;
    }
  }

  // Force update ALL workers to a specific location (ignores existing locations)
  async forceUpdateAllWorkersToLocation(lat: number, lng: number): Promise<{ updated: number; errors: number }> {
    this.logger.warn(`FORCE updating ALL workers to location (lat=${lat}, lng=${lng}) - existing locations will be overwritten!`);
    ...
    try {
      ...
      this.logger.debug(`Found ${workers.length} active workers to forcefully update`);
      ...
      for (const worker of workers) {
        try {
          ...
          await this.workerRepository.save(worker);
          updated++;
          this.logger.log(`Force updated worker ${worker.id}: (${oldLat}, ${oldLng}) -> (${lat}, ${lng})`);
        } catch (error) {
          errors++;
          this.logger.error(`Failed to force update worker ${worker.id}: ${error.message}`);
        }
      }

      this.logger.log(`FORCE worker location update complete: ${updated} updated, ${errors} errors`);
      return { updated, errors };
    } catch (error) {
      this.logger.error(`Error force updating all workers: ${error.message}`, error.stack);
      throw error;
    }
  }

  // Update service area bounds
  async updateServiceAreaBounds(areaId: string, minLat: number, maxLat: number, minLng: number, maxLng: number): Promise<ServiceArea> {
    this.logger.log(`Updating service area ${areaId} bounds: minLat=${minLat}, maxLat=${maxLat}, minLng=${minLng}, maxLng=${maxLng}`);

    try {
      const area = await this.serviceAreaRepository.findOne({ where: { id: areaId } });
      if (!area) {
      ...
      if (error instanceof NotFoundException || error instanceof BadRequestException) throw error;
      this.logger.error(`Error updating service area bounds: ${error.message}`, error.stack);
      throw error;
    }
  }

  // Expand service area to cover a specific location
  async expandServiceAreaToCoverLocation(lat: number, lng: number, paddingKm: number = 5): Promise<ServiceArea> {
    this.logger.log(`Expanding service area to cover location (${lat}, ${lng}) with ${paddingKm}km padding`);

    try {
      // Find the service area (or create one if none exists)
      let area = await this.serviceAreaRepository.findOne({ where: { isActive: true } });
      ...
      const saved = await this.serviceAreaRepository.save(area);
      this.logger.log(`Expanded service area ${area.id} to cover (${lat}, ${lng}). New bounds: minLat=${newMinLat}, maxLat=${newMaxLat}, minLng=${newMinLng}, maxLng=${newMaxLng}`);
      return saved;
    } catch (error) {
      this.logger.error(`Error expanding service area: ${error.message}`, error.stack);
      throw error;
    }
  }
}
