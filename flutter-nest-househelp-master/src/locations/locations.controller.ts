...
export class LocationsController {
  ...
  async checkAvailability(
    ...
    this.logger.log(`Checking availability at lat=${lat}, lng=${lng}, radius=${radius}km`);
  ...
  async getAvailableServices(
    ...
    this.logger.log(`Getting available services at lat=${lat}, lng=${lng}, radius=${radius}km`);
  ...
  async getNearbyZones(
    ...
    this.logger.debug(`Getting nearby zones for lat=${lat}, lng=${lng}, maxRadius=${maxRadius}km`);
  ...
  async addToWaitlist(
    ...
    this.logger.log(`Adding user to waitlist: userId=${waitlistDto.userId}, serviceId=${waitlistDto.serviceId}`);
  ...
  async getWaitlistStatus(@Req() req, @Param('userId') userId: string) {
    ...
    if (req.user.userId !== userId) {
      this.logger.warn(`Access denied: user ${req.user.userId} tried to access waitlist for user ${userId}`);
  ...
  async updatePreferredLocation(
    ...
    if (req.user.userId !== userId) {
      this.logger.warn(`Access denied: user ${req.user.userId} tried to update location for user ${userId}`);
      return { message: 'Access denied' };
    }
  ...
  async updateWorkerLocation(
    ...
    if (req.user.workerId !== workerId) {
      this.logger.warn(`Access denied: worker ${req.user.workerId} tried to update location for worker ${workerId}`);
      return { message: 'Access denied' };
    }
  ...
  async forceUpdateAllWorkerLocations(@Body() body: { lat: number; lng: number }) {
    ...
    return this.locationService.forceUpdateAllWorkersToLocation(body.lat, body.lng);
  }
  ...
  async debugLocationInfo(
    ...
    // Get service areas
    const serviceAreas = await this.locationService.getServiceAreasForLocation(lat, lng);
    
    // Get available workers
    const availableWorkers = await this.locationService.findAvailableWorkers(lat, lng, 5);
    
    return {
      deviceLocation: { lat, lng },
      nearbyZones: nearbyZones.map(z => ({ id: z.id, name: z.name, centerLat: z.centerLat, centerLng: z.centerLng })),
      serviceAreas: serviceAreas.map(a => ({ id: a.id, name: a.name, minLat: a.minLat, maxLat: a.maxLat, minLng: a.minLng, maxLng: a.maxLng })),
  ...
  async expandServiceArea(@Body() body: { lat: number; lng: number; paddingKm?: number }) {
    ...
    return this.locationService.expandServiceAreaToCoverLocation(body.lat, body.lng, body.paddingKm || 5);
  }
}
