import { IsEmail, IsString, IsNotEmpty, IsOptional, IsEnum, IsNumber, MinLength, MaxLength, Matches, ValidateIf } from 'class-validator';
import { UserRole } from '../entities/user.entity';
import { IsValidRoleAssignment } from '../../common/validators/role-constraint.validator';

export class AdminCreateUserDto {
    @IsEmail({}, { message: 'Please provide a valid email address in the correct format (e.g., user@example.com)' })
    ...
    @IsValidRoleAssignment({ message: 'Invalid role assignment for admin operations' })
    role: UserRole;
    ...
    @Matches(/^\+?[1-9]\d{1,14}$/, { message: 'Please provide a valid phone number in international format (e.g., +1234567890)' })
    phone?: string;
