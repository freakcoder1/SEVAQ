...
export class DatabaseMonitoringService implements OnModuleInit {
  constructor(
    ...
    @InjectMetric('database_active_connections') public activeConnections: Gauge<string>,
    @InjectMetric('database_idle_connections') public idleConnections: Gauge<string>,
    @InjectMetric('database_total_connections') public totalConnections: Gauge<string>,
    @InjectMetric('database_query_execution_time_seconds') public queryExecutionTime: Histogram<string>,
    @InjectMetric('database_slow_queries_total') public slowQueries: Gauge<string>,
  ) {}
  ...
  private async updateConnectionPoolMetrics() {
    try {
      ...
      const activeConnections = this.dataSource.queryResultCache ?
        Object.keys(this.dataSource.queryResultCache).length : 0;
      this.activeConnections.set(activeConnections);

      // Get idle connections (approximation)
      const idleConnections = Math.max(0,
      ...
      console.log(`Database metrics updated: active=${activeConnections}, idle=${idleConnections}`);
