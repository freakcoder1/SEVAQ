...
describe('SlotsService', () => {
  ...
  describe('findAvailableSlot', () => {
    it('should find an available slot with exact match', async () => {
      ...
      const result = await service.findAvailableSlot(
        'worker-1',
        startTime,
        endTime,
      );
  ...
  describe('findAvailableSlotFlexible', () => {
    it('should find exact match first', async () => {
      const startTime = new Date('2024-01-15T10:00:00.000Z');
      const endTime = new Date('2024-01-15T14:00:00.000Z');
      const exactMatch = createMockSlot(
        'slot-1',
        'worker-1',
        startTime,
        endTime,
      ...
      expect(service.findAvailableSlot).toHaveBeenCalledWith(
        'worker-1',
        startTime,
        endTime,
      );
    ...
    it('should find flexible match within 30-minute window', async () => {
      ...
      const flexibleSlot = createMockSlot(
        'slot-2',
        'worker-1',
        flexibleStart,
        flexibleEnd,
    ...
    it('should find same-day slot when no flexible match', async () => {
      ...
      const sameDaySlot = createMockSlot(
        'slot-3',
        'worker-1',
        sameDayStart,
        sameDayEnd,
  ...
  describe('createSlotsForWorker', () => {
    ...
    it('should throw error if worker not found', async () => {
      ...
      await expect(
        service.createSlotsForWorker(workerId, date, timeSlots),
      ).rejects.toThrow('Worker with ID worker-999 not found');
    });
  });
  ...
  describe('getAvailableSlotsForWorker', () => {
    it('should return available slots within date range', async () => {
      ...
      const result = await service.getAvailableSlotsForWorker(
        workerId,
        startDate,
        endDate,
      );
