...
async function main() {
  ...
  if (!workersRes.data || !Array.isArray(workersRes.data) || workersRes.data.length === 0) {
    console.log('No workers found or invalid response:', workersRes.data);
    return;
  }
  ...
  const logs = logsData.data || logsData; // Handle {data: [...]} or [...]
  const total = logsData.total || (Array.isArray(logsData) ? logsData.length : 0);
